...
public struct BottomBar : View {
    ...
    public var body: some View {
        ...
        .padding()
        .animation(.default).background(colorScheme == .dark ? Color.black : Color.white)
    }
}
...
struct BottomBar_Previews : PreviewProvider {
    static var previews: some View {
        BottomBar(selectedIndex: .constant(0), items: [
            BottomBarItem(icon: "house.fill", title: "Discover", color: .purple),
            BottomBarItem(icon: "heart", title: "Likes", color: .pink),
            BottomBarItem(icon: "magnifyqingglass", title: "Search", color: .orange),
